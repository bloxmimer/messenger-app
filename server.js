...
let currentLang='ru';
function updateTexts(){document.getElementById('enterEmailText')&&(document.getElementById('enterEmailText').innerText=translations[currentLang].enterEmail);document.getElementById('getCodeBtn')&&(document.getElementById('getCodeBtn').innerText=translations[currentLang].getCode);document.getElementById('enterCodeText')&&(document.getElementById('enterCodeText').innerText=translations[currentLang].enterCode);document.getElementById('loginBtn')&&(document.getElementById('loginBtn').innerText=translations[currentLang].login);document.getElementById('chatsTitle')&&(document.getElementById('chatsTitle').innerText=translations[currentLang].chats);document.getElementById('messageInput')&&(document.getElementById('messageInput').placeholder=translations[currentLang].message);}
function setTheme(t){currentTheme=t;if(t==='dark')document.body.classList.add('dark');else document.body.classList.remove('dark');localStorage.setItem('theme',t);if(currentUser){fetch(API+'/users/update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:currentUser.id,theme:t})});}}
function setLang(l){currentLang=l;localStorage.setItem('lang',l);updateTexts();if(currentUser){fetch(API+'/users/update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:currentUser.id,lang:l})});}}
